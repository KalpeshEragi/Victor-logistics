...
import { Cabin, cabins, formatPrice, getRouteBase } from "@/data/cabins";

interface CabinDetailProps {
  cabin: Cabin;
}

export default function CabinDetail({ cabin }: CabinDetailProps) {
  const routeBase = getRouteBase(cabin.builtType);
  const categoryLabel =
    cabin.builtType === "Prefab" ? "Prefab Cabins" : "Fabricated Cabins";

  // Related cabins: same builtType, excluding current
  const relatedCabins = cabins
    .filter((c) => c.builtType === cabin.builtType && c.id !== cabin.id)
    .slice(0, 3);
      ...
      <div className="bg-[#f8faf8] border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-6 py-4">
          <nav className="flex items-center gap-2 text-sm text-gray-500">
            ...
            <span>/</span>
            <Link href={routeBase} className="hover:text-green-600">
              {categoryLabel}
            </Link>
            <span>/</span>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-8">
                {(
                  [
                    ["Size", cabin.size],
                    ["Material", cabin.material],
                    ["Built Type", cabin.builtType],
                    ["Feature", cabin.feature],
                    ["Color", cabin.color],
                    ["Delivery", cabin.deliveryTime],
                  ] as [string, string][]
                ).map(([label, value]) => (
                  <div key={label} className="bg-[#f8faf8] rounded-xl p-4">
      ...
      <section className="bg-white border-t border-gray-100">
        <div className="max-w-5xl mx-auto px-6 py-14 text-center">
          ...
          <p className="text-gray-600 mb-6">
            Get the best price and fast delivery for your business needs.
          </p>
          <Link
            href="/contact"
